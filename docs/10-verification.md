...
Carbon histogram port（`BucketFor` / `BucketUpper`）は全bucket境界の100 vectorsと、seed 20260925の10,000 random vectorsでC++ oracleとbyte一致します（[port evidence](evidence/carbon-histogram.md)、[differential evidence](evidence/carbon-differential.md)、`./scripts/run-carbon-differential.sh`）。CIの`Carbon labs (pinned nightly)` jobが`./scripts/check-carbon.sh`でCarbon checkをすべて実行し、`carbon-nightly` workflowが新しいnightlyを同じcheckで毎日検証します（[手順](13-carbon-nightly-refresh.md)）。
